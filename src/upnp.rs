...
impl UpnpClient {
    ...
    pub async fn discover_device(&mut self) -> Result<()> {
        ...
        socket.send_to(UPNP_SEARCH_MSG.as_bytes(), UPNP_MULTICAST_ADDR).await?;
        ...
        match tokio::time::timeout(Duration::from_secs(5), socket.recv_from(&mut buf)).await {
            Ok(Ok((len, _addr))) => {
                ...
                if let Some(location) = self.extract_location(&response) {
                    info!("Found UPnP device at: {}", location);
    ...
    async fn setup_service(&mut self) -> Result<()> {
        let device = self.device.as_ref().ok_or_else(|| anyhow!("No device found"))?;
    ...
    fn parse_service_urls(&self, xml: &str, _base_url: &str) -> Result<(Option<String>, Option<String>)> {
                    ...
                    match name.local_name.as_str() {
                        "service" => {
                            if current_service_type.contains("WANCommonInterfaceConfig") {
                                ...
                                info!("Found WANCommonInterfaceConfig service at: {}", full_url);
                                ...
                                info!("Found WANIPConnection service at: {}", full_url);
    ...
    pub async fn get_traffic_stats(&self) -> Result<TrafficStats> {
        let device = self.device.as_ref().ok_or_else(|| anyhow!("No device configured"))?;
        let wan_common_url = device.wan_common_service_url.as_ref().ok_or_else(|| anyhow!("No WANCommonInterfaceConfig service URL"))?;
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    <s:Body>
        ...
        let response = self.soap_request(service_url, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetTotalBytesSent", soap_body).await?;
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    <s:Body>
        ...
        let response = self.soap_request(service_url, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetTotalBytesReceived", soap_body).await?;
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    <s:Body>
        ...
        let response = self.soap_request(service_url, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetTotalPacketsSent", soap_body).await?;
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    <s:Body>
        ...
        let response = self.soap_request(service_url, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetTotalPacketsReceived", soap_body).await?;
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
    <s:Body>
        ...
        let response = self.soap_request(service_url, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1#GetCommonLinkProperties", soap_body).await?;
        self.parse_string_response(&response, "NewPhysicalLinkStatus")
    }
    
    async fn soap_request(&self, service_url: &str, soap_action: &str, soap_body: &str) -> Result<String> {
        ...
        loop {
            match reader.next() {
                ...
                Ok(XmlEvent::Characters(text)) if in_target_element => {
                    return text.parse::<u64>().map_err(|e| anyhow!("Failed to parse {}: {}", element_name, e));
                }
                Ok(XmlEvent::EndElement { name }) => {
                    if name.local_name == element_name {
